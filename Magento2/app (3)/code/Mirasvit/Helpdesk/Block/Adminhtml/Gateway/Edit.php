...
class Edit extends \Magento\Backend\Block\Widget\Form\Container
{
    ...
    {
        if ($this->registry->registry('current_gateway') && $this->registry->registry('current_gateway')->getId()) {
            return $this->registry->registry('current_gateway');
        }
    }
    ...
    public function getHeaderText()
    {
        if ($gateway = $this->getGateway()) {
            return __("Edit Gateway '%1'", $this->escapeHtml($gateway->getName()));
        } else {
            return __('Create New Gateway');
        }
    }
