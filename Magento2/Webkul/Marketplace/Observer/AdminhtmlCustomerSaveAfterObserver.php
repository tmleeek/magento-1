    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($this->isSeller($customerid)) {
            ...
            if (empty($errors)) {
                ...
                if(count($postData['company_locality'])>0)
                    {
                        $postData['company_locality']=implode(",",array_filter($postData['company_locality']));
                         
                    }
                    else{

                        $this->_messageManager->addError("Postalcode is required.");
                        return $this;
                    }
