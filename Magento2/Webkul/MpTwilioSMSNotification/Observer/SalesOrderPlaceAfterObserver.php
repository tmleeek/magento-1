...
/**
 * Webkul MpTwilioSMSNotification SalesOrderPlaceAfterObserver Observer.
 */
class SalesOrderPlaceAfterObserver implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        if ($this->_helperData->getTwilioStatus()) {
            ...
            $orderIncrementedId = $observer->getOrder()->getIncrementId();
            ...
            foreach ($sellerOrder as $info) {
                if ($info->getSellerId()!=0) {
                    ...
                    foreach ($mpSellerCollection as $sellerData) {
                        ...
                        $content = __(
                            "Hi %1, the Order %2 has been placed, please check your mail for more details at %3",
                            $sellerName,
                            $orderIncrementedId,
                            $userData->getEmail()
                        );
