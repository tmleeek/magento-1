...
/**
 * Webkul MpTwilioSMSNotification SalesOrderCreditmemoRefundObserver Observer.
 */
class SalesOrderCreditmemoRefundObserver implements ObserverInterface
{
    ...
    /**
     * sales_order_creditmemo_refund event handler.
     *
            ...
            foreach ($sellerOrder as $info) {
                if ($info->getSellerId()!=0) {
                    ...
                    foreach ($mpSellerCollection as $sellerdata) {
                        ...
                        $content = __(
                            "Hi %1,the Order's creditmemo Generated,please check your mail for more details at %2",
                            $sellerName,
                            $userData->getEmail()
                        );
                        try {
