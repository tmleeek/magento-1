...
class MassChange extends \Mirasvit\Helpdesk\Controller\Adminhtml\Pattern
{
    /**
     *
     */
    public function execute()
    {
        ...
        if (!is_array($ids)) {
            $this->messageManager->addError(__('Please select Pattern(s)'));
        } else {
            try {
                $isActive = $this->getRequest()->getParam('is_active');
                foreach ($ids as $id) {
                    /** @var \Mirasvit\Helpdesk\Model\Pattern $pattern */
                    $pattern = $this->patternFactory->create()->load($id);
