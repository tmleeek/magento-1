...
class Save extends \Mirasvit\Helpdesk\Controller\Adminhtml\Schedule
{
    ...
    public function execute()
    {
        if ($data = $this->getRequest()->getParams()) {
            ...
            if (!$data['working_hours']) {
                $this->messageManager->addError(__('Working days/hours is a required field.'));
            ...
            try {
                ...
                $this->messageManager->addSuccess(__('Schedule was successfully saved'));
        ...
        $this->messageManager->addError(__('Unable to find Schedule to save'));
        $this->_redirect('*/*/');
    }
    ...
    protected function prepareSchedule($data)
    {
        ...
        if ($data['type'] == \Mirasvit\Helpdesk\Model\Config::SCHEDULE_TYPE_CUSTOM) {
            ...
            foreach ($data['working_time_day'] as $day) {
                $schedule[$day] = [
