...
class Edit extends \Magento\Backend\Block\Widget\Form\Container
{
    ...
    {
        if ($this->registry->registry('current_schedule')
            && $this->registry->registry('current_schedule')->getId()) {
            return $this->registry->registry('current_schedule');
        }
    }
    ...
    public function getHeaderText()
    {
        if ($schedule = $this->getSchedule()) {
            return __("Edit Schedule '%1'", $this->escapeHtml($schedule->getName()));
        } else {
            return __('Create New Schedule');
        }
    }
