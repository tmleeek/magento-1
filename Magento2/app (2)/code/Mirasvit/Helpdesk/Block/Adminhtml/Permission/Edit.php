...
class Edit extends \Magento\Backend\Block\Widget\Form\Container
{
    ...
    {
        if ($this->registry->registry('current_permission') && $this->registry->registry(
            'current_permission'
        )->getId()) {
            return $this->registry->registry('current_permission');
        }
    }
    ...
    public function getHeaderText()
    {
        if ($permission = $this->getPermission()) {
            return __('Edit Permission "%1"', $this->escapeHtml($permission->getName()));
        } else {
            return __('Create New Permission');
        }
    }
