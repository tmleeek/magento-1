...
class User extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getHelpdeskUser()
    {
        ...
        try {
            ...
            $query = 'SELECT COUNT(*) FROM '.$resource->getTableName('mst_helpdesk_user').
                ' WHERE user_id='.$userId;
            if ($resource->getConnection('core_read')->fetchOne($query) == 0) {
                $query = 'INSERT INTO '.$resource->getTableName('mst_helpdesk_user').
                    ' (user_id) VALUES ('.$userId.')';
                $resource->getConnection('core_write')->query($query);
            }
        } catch (\Exception $e) {
            //it's possible that migrations were not run at this point. Table has not been created yet.
            $this->context->getLogger()->alert($e->getMessage());

            return false;
        }
        $helpdeskUser = $this->userFactory->create()->load($userId);
